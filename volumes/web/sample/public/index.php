<?php
    ...
    switch($mode){
        case "create":
            ...
            $created_todo = [
                "title"       => ((new Simnet\Todo\Title($title_raw))->get()),
                "finished_at" => ((new Simnet\Todo\FinishedAt($finished_at_raw))->get())
            ];
        ...
        case "update":
            ...
            $updated_todo    = [
                "title"       => ((new Simnet\Todo\Title($title_raw))->get()),
                "finished_at" => ((new Simnet\Todo\FinishedAt($finished_at_raw))->get()),
                "status"      => ((new Simnet\Todo\Status($status_raw))->get())
            ];
    ...
    $todo_list_picked_by_status = $TODO->pickTodoByCheckedStatus(Simnet\Todo\Status::pickStatusByChecked($checked_status_to_be_displayed), $TODO->read());
